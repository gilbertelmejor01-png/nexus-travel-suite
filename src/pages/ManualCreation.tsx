...
const ManualCreation = () => {
  ...
  const calcularCostoEstimado = () => {
    ...
    ciudades.forEach(ciudad => {
      if (ciudad.fechaEntrada && ciudad.fechaSalida) {
        const noches = Math.ceil((ciudad.fechaSalida.getTime() - ciudad.fechaEntrada.getTime()) / (1000 * 60 * 60 * 24));
        totalNoches += noches;
      }
    });
  ...
  const enviarCliente = () => {
    ...
    toast({
      ...
      description: "El presupuesto ha sido enviado al cliente exitosamente"
    });
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground">Crea presupuestos personalizados paso a paso</p>
      </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="text-center p-4 bg-gradient-subtle rounded-lg">
                ...
                <h3 className="font-medium">Viaje a {pais || "País por seleccionar"}</h3>
              ...
              {ciudades.filter(c => c.ciudad).length > 0 && (
                <div className="space-y-2">
                  ...
                  {ciudades.filter(c => c.ciudad).map((ciudad, index) => (
                    <div key={ciudad.id} className="text-xs p-2 bg-muted rounded">
                      <p className="font-medium">{index + 1}. {ciudad.ciudad}</p>
                      {ciudad.hotel && <p>🏨 {ciudad.hotel}</p>}
                      {ciudad.fechaEntrada && ciudad.fechaSalida && (
                        <p>📅 {format(ciudad.fechaEntrada, "dd/MM")} - {format(ciudad.fechaSalida, "dd/MM")}</p>
                      )}
                    </div>
                  ))}
                </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              <div className="flex items-center justify-between text-sm">
                ...
                <Badge variant={ciudades.every(c => c.ciudad && c.hotel) ? "default" : "secondary"}>
                  {ciudades.filter(c => c.ciudad && c.hotel).length}/{ciudades.length}
                </Badge>
              </div>
              <div className="flex items-center justify-between text-sm">
                <span>Fechas configuradas</span>
                <Badge variant={ciudades.every(c => c.fechaEntrada && c.fechaSalida) ? "default" : "secondary"}>
                  {ciudades.filter(c => c.fechaEntrada && c.fechaSalida).length}/{ciudades.length}
                </Badge>
              </div>
            </CardContent>
          </Card>
            ...
            <Card className="border-success">
              <CardContent className="pt-6">
                <div className="text-center">
                  ...
                  <p className="text-sm text-muted-foreground">
                    El presupuesto ha sido enviado al cliente
                  </p>
                </div>
              </CardContent>
            </Card>
